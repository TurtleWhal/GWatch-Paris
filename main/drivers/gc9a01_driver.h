...
#define GC9A01_PIN_BLK 2 // Set to -1 if not used
...
void gc9a01_cleanup(void)
{
    ...
    if (gc9a01.dma_mutex)
    {
        vSemaphoreDelete(gc9a01.dma_mutex);
        gc9a01.dma_mutex = NULL;
    }

    gc9a01.dma_enabled = false;
    ESP_LOGI(TAG, "GC9A01 cleanup completed");
}
