...
for symbol in symbols["symbols"]:
    # Use 'char' if it exists, otherwise use hex
    ...
    symboldefines.append(
        f"#define FA_{(symbol['name'].replace('-', '_').replace(' ', '_').upper().ljust(longest, ' '))} {char_value} {hex_comment}\n"
    )
    for size in symbol["sizes"]:
        if symbol.get("brand") is None:
            if "char" in symbol and symbol["char"]:
                sizes[size].append(f"{ord(symbol['char']):04X}")
            else:
                sizes[size].append(symbol["hex"])
        else:
            if "char" in symbol and symbol["char"]:
                sizes[size].append(f"{ord(symbol['char']):04X}" + "\0")
            else:
...
with open(
    os.path.realpath(
        pathlib.Path(__file__).parent.resolve().joinpath("./generated_sources.cmake")
    ),
    "w",
) as cmake_file:
